...
/**
 * Created by Engr.Uzma on 25/08/2016.
 */
...
public final class QueryUtils {
     ...
     */
    public static String USGS_REQUEST_URL="http://earthquake.usgs.gov/fdsnws/event/1/query";
    ...
    /**
     * Return a list of {@link ContentClass} objects that has been built up from
     * parsing a JSON response.
     */
    public static ArrayList<ContentClass> extractContentClasss(String SAMPLE_JSON_RESPONSE) {

        // Create an empty ArrayList that we can start adding ContentClasss to
        ArrayList<ContentClass> earthquick = new ArrayList<>();
        ...
        try {
            ...
            JSONArray arrayOFearthQuaks=root.getJSONArray("features");

            for (int i=0;i<arrayOFearthQuaks.length();i++)
            {
                JSONObject jsonObject=arrayOFearthQuaks.getJSONObject(i).getJSONObject("properties");

                double magnitude=jsonObject.getDouble("mag");
                String placed=String.valueOf(jsonObject.getString("place"));
                long time=jsonObject.getLong("time");
                String url=jsonObject.getString("url");



                earthquick.add(new ContentClass(magnitude,placed,time,url));
